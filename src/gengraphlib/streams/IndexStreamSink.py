...
class IndexStreamSink:

    def __init__( self: Self, bootlog_info: BootLogInfo, end_event: mp.Event, write_bin: bool, write_log: bool ) -> None:
        ...
        self.keyvalue_list: KeyRecordList = []
    ...
    async def spin_stream( self: Self, active_keys: set[str], record_queue: mp.Queue ) -> None:
        ...
        self.valuemux_queue = record_queue
        ...
        self.cmd_stream = CmdStdoutStream(f"/bin/journalctl -b {self.bootlog_info.boot_index} -o export", self.end_event )
    ...
    def recv_line( self: Self, line: str ) -> None:
        ...
        if len(line) > 2:
            ...
            if key in self.active_keys:
                self.keyvalue_list.append( (key, value) )
        else:

            keyrecord_packet: KeyRecordPacket = ( ++self.record_count, self.keyvalue_list )
            self.valuemux_queue.put( keyrecord_packet )
            self.keyvalue_list = []
