...
class LineNodes(NodeBase, list[LogLineNode]):

    def __init__( self: Self ) -> None:
        #self.log_file_graph: LogFileGraph
        self.cnt: int = 0
        super(LineNodes, self).__init__(id="lineNodeIndex")

    def new_line( self: Self, line_str: str, line_num: int ) -> LogLineNode:
        new_line: LogLineNode =  LogLineNode( line_str=line_str, line_num=line_num )
        self.append( new_line )
        return new_line

    def __add__(self, other: LogLineNode) -> None:
        self.append(other)
